...
- **FEAT-J** 仿真翻页模式（page-flip 库 + 浮动进度指示器）
- **FEAT-K** 并行扫描加速（rayon par_iter + Triangle 缩放）
- **FEAT-L** 文件夹导入模式（子目录自动识别为系列 + scan_mode 持久化）
- **FEAT-M** 动态背景渐变（fast-average-color 封面主色提取）
- **FEAT-N** Everything 全盘搜索集成（es.exe + 勾选导入）
- **FEAT-O** 收藏文件夹自动监控（notify crate + books-updated 事件）
- **FEAT-P** 白边裁剪（CSS clipPath inset + scale）
- **FEAT-Q** 自动载入下一文件（同系列按标题排序 + get_next_book 命令）
- **FEAT-R** 自定义阅读器背景色（6 预设色 + 自定义调色盘）
...
| 功能 | 说明 | 状态 |
|---|---|---|
| 文件夹导入模式 | 导入父文件夹自动识别子文件夹中的漫画并归纳为系列 | ✅ 已完成 |
| 漫画加载优化 | 图片列表缓存、内存 LRU、自动磁盘清理策略 | 方案设计中 |
| 仿真翻页动画 | 模拟真实书页翻转效果 + 进度位置优化 | ✅ 已完成 |
| 首页动态背景 | 书库封面+主色渐变背景（低透明度） | ✅ 已完成 |
| Everything 搜索集成 | 接入 Everything SDK 搜索本地漫画文件 | ✅ 已完成 |
| 收藏文件夹 | 监控指定文件夹，自动导入新增漫画 | ✅ 已完成 |
| github action cd集成 发布release| | 用户自行配置 |
| 白边裁剪 | 修剪图像边缘的空白区域，以便于图片更易阅读。| ✅ 已完成 |
| 自动载入下一个文件（如果存在） 默认开启| | ✅ 已完成 |
| 自定义阅读器背景颜色,通过调色盘或吸管获取颜色| | ✅ 已完成 |

### 发现bug
1. ~~加载漫画速度过慢~~ → ✅ 已修复：rayon 并行扫描 + Triangle 缩放替代 Lanczos3
2. ~~webdav ui样式不一致~~ → ✅ 已修复：输入框 bg 改为主题变量
3. ~~锐化,色彩增强,文字增强全部不生效~~ → ✅ 已修复：filter 值加强 + 双页模式传递
4. uiux页面统一,删除不必要的卡片圆角